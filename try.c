...
void closeFD(int FD)
{
	do
	{
		...
		{
			perror("\033[0;32mAnanya\033[0;37m:\033[0;31m12040180\033[0;35m>>");
		}
	}while(0);
}																				
...
int runRedirection(char ** commands, int j, int loc[])
{
	...
	for (; i<j; i++)
	{
        ...
        else if ((strcmp(commands[loc[i]], "&") == 0) && (strcmp(commands[loc[i+1]], "&") == 0)) 
        { 
            printf("hi");
			in = open(commands[loc[i]+2], O_RDONLY);
   			dup2(in, 0);
   			close(in);
   			end[loc[i]] = NULL;
			if (execvp(end[0], end) == -1)
...
//     while (RUN_STATUS) {
//         printf("chaitanya_bisht@12040450 $ ");
//         fflush(stdout);
	...
	else
	{
		...
		if (pid == 0)
		{
			...
			else
			{
				if (execvp(commands[0], commands) == -1)
				{
					perror("\033[0;32mAnanya\033[0;37m:\033[0;31m12040180");
					exit(EXIT_FAILURE);
				}
			}
		}
...
int runShell(char **commands)
{
	...
	else if(strcmp(commands[0], "cd") == 0)
	{
		...
		{
			perror("\033[0;32mAnanya\033[0;37m:\033[0;31m12040180\033");
		}
		status = 1;
	}
	else
